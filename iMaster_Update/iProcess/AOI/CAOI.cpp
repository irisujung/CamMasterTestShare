...
UINT32 CAOI::SaveProductInfo(IN INT32 iStepIdx, IN enum Orient eOrient)
{
	...
	for (auto itMark : m_stPanelInfo.vcAlignMarkInfo)
	{
		...
		//22.02.10, 0도 좌상단 기준으로 좌표 계산 요청 반영
...
UINT32 CAOI::SetAlignCrossPoint(IN const INT32 & iStepIdx)
{
	...
	//같은 Feature 끼리 묶어 주었으니,
	int nAlignCount = static_cast<int>(vecvecAlignIndex.size());
	if (nAlignCount > 0)
	{
		CString str;
		str.Format(_T("Auto Search로 찾은 Align Point가 있습니다.(Pattern:%d)\n 기존 Align 포인트는 삭제하고 대체 합니다."), nAlignCount);
		AfxMessageBox(str);
	...
	{
		AfxMessageBox(_T("Auto Search로 찾은 Align Point가 없습니다."));
	}
...
UINT32 CAOI::EstimateFOV_Based_On_Strip(INT32 iStepIdx, enum Orient eOrient)
{
	...
	else if (m_pSystemSpec->sysBasic.MasterLevel == eMasterLevel::ePanelLv)
	{
		...
		{
			strLog.Format(_T("Strip Row 개수는 %d, Block Row 개수는 %d입니다. Cell Row 개수는 %d의 배수가 되어야 합니다. 입력 값을 확인하여 주십시오."),
		...
		if (iRowCellNumInBlock == 0)
		{
			strLog.Format(_T("Strip Row 개수는 %d, Block Row 개수는 %d입니다. Cell Row 개수는 %d보다 커야 합니다. 입력 값을 확인하여 주십시오."),
				iRowStripNumInPanel, iRowBlockNumInStrip, iRowBlockNumInStrip * iRowStripNumInPanel);
			AfxMessageBox(strLog);
			return RESULT_BAD;
		}
	...
	if (m_pSystemSpec->sysBasic.MasterLevel == eMasterLevel::eBlockLv)
	{
		...
		{
			strLog.Format(_T("Block Swath 개수는 %d입니다. Cell Col 개수는 %d의 배수가 되어야 합니다. 입력 값을 확인하여 주십시오."),
				iSwathNum, iSwathNum);
			AfxMessageBox(strLog);
			return RESULT_BAD;
		}
		...
		if (iColCellNumInBlock == 0)
		{
			strLog.Format(_T("Block Swath 개수는 %d입니다. Cell Col 개수는 %d보다 커야 합니다. 입력 값을 확인하여 주십시오."),
				iSwathNum, iSwathNum);
			AfxMessageBox(strLog);
			return RESULT_BAD;
		}
...
UINT32 CAOI::CheckParams()
{
	...
	if (dResolution <= 0)
	{
		AfxMessageBox(_T("해상도 입력 값이 올바르지 않습니다."));
		return RESULT_BAD;
	}
	...
	if (iFrameWidth <= 0)
	{
		AfxMessageBox(_T("Frame Width 입력 값이 올바르지 않습니다."));
		return RESULT_BAD;
	}
	...
	if (iCellColNum <= 0)
	{
		AfxMessageBox(_T("Cell Col 개수 입력 값이 올바르지 않습니다."));
		return RESULT_BAD;
	}
	...
	if (iCellRowNum <= 0)
	{
		AfxMessageBox(_T("Cell Row 개수 입력 값이 올바르지 않습니다."));
		return RESULT_BAD;
	}
