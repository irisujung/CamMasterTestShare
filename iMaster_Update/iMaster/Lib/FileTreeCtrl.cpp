...
Copyright (c) 1999 - 2013 by PJ Naughter (Web: www.naughter.com, Email: pjna@naughter.com)
...
CSystemImageList::CSystemImageList()
{
  ...
  SHFILEINFO sfi;
  HIMAGELIST hSystemImageList = reinterpret_cast<HIMAGELIST>(SHGetFileInfo(szTempDir, 0, &sfi, sizeof(SHFILEINFO), SHGFI_SYSICONINDEX | SHGFI_SMALLICON));
...
HTREEITEM CFileTreeCtrl::SetSelectedPath(const CString& sPath, BOOL bExpanded)
{
  ...
  if (hItemFound)
  {
	...
	if (hItemFound == TVI_ROOT) return hItemFound;
    SelectItem(hItemFound);
    if (bExpanded)
    {
      BOOL bHasPlus = HasPlusButton(hItemFound);
...
//  BOOL bSuccess = FALSE;
  if (m_bAllowOpen && hItem)
  {
	...
	m_sSelectFileName = ItemToPath(hItem);
	if (m_sSelectFileName.Find(_T(".tgz")) > 0 || m_sSelectFileName.Find(_T(".zip")) > 0)
		GetParent()->SendMessage(WM_COMMAND, TREE_NODE_CLICK);
...
BOOL CFileTreeCtrl::IsUNCServerName(const CString& sPath)
{
  TCHAR szDrive[_MAX_DRIVE];
  szDrive[0] = _T('\0');
  TCHAR szDir[_MAX_DIR];
...
BOOL CFileTreeCtrl::EnumNetwork(NETRESOURCE* pParentNetResource, HTREEITEM hParent)
{
  //What will be the return value from this function
  ...
  {
    TRACE(_T("CFileTreeCtrl::EnumNetwork, Cannot enumerate network, Error:%d\n"), dwResult);
    return FALSE;
  } 
...
HTREEITEM CFileTreeCtrl::DisplayNetworkComputer(HTREEITEM hParent, const CString& sComputer)
{
  //Setup the item data for the new item
  ...
  #ifndef FILETREECTRL_NO_CACHE_COMPUTER_ICON    
    if ((m_nComputerIconIndex != -1) && (m_nSelectedComputerIconIndex != -1))
      hItem = InsertFileItem(hParent, pNewItem, FALSE, m_nComputerIconIndex, m_nSelectedComputerIconIndex, FALSE);
    else
    {
      CString sServer;
      sServer.Format(_T("\\\\%s"), pNewItem->m_sRelativePath.operator LPCTSTR());
      hItem = InsertFileItem(hParent, pNewItem, FALSE, GetIconIndex(sServer), GetSelIconIndex(sServer), FALSE);
    }
  #else
    CString sServer;
    sServer.Format(_T("\\\\%s"), pNewItem->m_sRelativePath.operator LPCTSTR());
...
BOOL CFileTreeCtrl::DisplayShares(HTREEITEM hParent, const CString& sComputer)
{
...
void CFileTreeCtrl::DisplayPath(const CString& sPath, HTREEITEM hParent, BOOL bUseSetRedraw, BOOL bDeleteExistingSiblings)
{
  ...
  //if (GetCurCamDataType() == eODB)
  //{
	  //Add ODB++ files
	  if (m_bRecursiveSub)
		  AddRecursiveCamFile(sPath, hParent, _T(".tgz"));
	  else
		  AddCamFile(sPath, hParent, _T(".tgz"));
  //}
  //else if (GetCurCamDataType() == eGerber)
  //{
	 // //Add Gerber files
	 // AddCamFile(sPath, hParent, _T(".zip"));
  //}
  //else if (GetCurCamDataType() == eDxf)
  //{
	 // //Add Dxf files
	 // AddCamFile(sPath, hParent, _T(".zip"));
  //}
...
void CFileTreeCtrl::AddCamFile(const CString& sPath, HTREEITEM hParent, CString strFileExt)
{
	...
	for (INT_PTR i = 0; i < nMasks; i++)
	{
		...
		while (bFind)
		{
			...
			if (!find2.IsDirectory())
			{
				...
				{
					++nFiles;
					CString sTempPath(find2.GetFilePath()), strExt;

					TCHAR szPath[_MAX_PATH];
					TCHAR szFname[_MAX_FNAME];
					TCHAR szExt[_MAX_EXT];
					_tsplitpath_s(sTempPath, NULL, 0, NULL, 0, szFname, sizeof(szFname) / sizeof(TCHAR), szExt, sizeof(szExt) / sizeof(TCHAR));
					strExt = (TCHAR*)(LPCTSTR)szExt;
					if (strExt.CompareNoCase(strFileExt)) continue;
	...
	if (nFiles == 0 && hParent != m_hRoot)
		SetHasPlusButton(hParent, FALSE);
}

void CFileTreeCtrl::AddRecursiveCamFile(const CString& sPath, HTREEITEM hParent, CString strFileExt)
{
	...
	while (bFind)
	{
		...
		if (find.IsDirectory())
		{
			if (CanDisplayFolder(find))
			{
				++nDirectories;
				CString sTempPath(find.GetFilePath());
	...
	for (INT_PTR i = 0; i<nMasks; i++)
	{
		if (sPath.GetAt(sPath.GetLength() - 1) == _T('\\'))
			...
			sFile = sPath + _T("\\") + m_sFileNameMasks.GetAt(i);
		bFind = find.FindFile(sFile);
		while (bFind)
		{
			bFind = find.FindNextFile();
			if (!find.IsDirectory())
			{
				if (CanDisplayFile(find))
				{
					++nFiles;
					CString sTempPath(find.GetFilePath()), strExt;
					...
					_tsplitpath_s(sTempPath, NULL, 0, NULL, 0, szFname, sizeof(szFname) / sizeof(TCHAR), szExt, sizeof(szExt) / sizeof(TCHAR));
					strExt = (TCHAR*)(LPCTSTR)szExt;
					if (strExt.CompareNoCase(_T(".tgz"))) continue;
