...
void CCustomTreeChildCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (CheckHit(point))
	{
		...
		if(hItem)
		{
			...
			m_bOldItemSelected = GetItemState(hItem, UINT(TVIS_SELECTED)) & TVIS_SELECTED;

			HTREEITEM hParentItem = this->GetParentItem(hItem);
			
			//Parent가  클릭되었으면 선택하지 않음
			if (hParentItem == NULL) return;

			CString strParentName = this->GetItemText(hParentItem);

			if (bCtrl == TRUE)
			{
				if (m_bOldItemSelected == TRUE)
				{
...
void CCustomTreeChildCtrl::SelectItems(HTREEITEM hFrom, HTREEITEM hTo) {
...
void CCustomTreeChildCtrl::ClearSelection(HTREEITEM hExcept, HTREEITEM hItem) {
	if (!hItem)
		hItem = GetRootItem();
	while (hItem) {
		if (hItem != hExcept)
...
BOOL CColumnTreeCtrl::GetSelectInfo(vector<CString> &vecParentName, vector<CString> &vecInfo)
{
	vecParentName.clear();
	vecInfo.clear();
	...
	while (1)
	{
		if (hRootItem == NULL) break;

		hItem = m_Tree.GetChildItem(hRootItem);
		if (hItem != NULL)
		{
			while (1)
			{
				if ((m_Tree.GetItemState(hItem, UINT(TVIS_SELECTED)) & TVIS_SELECTED) != 0)
				{//선택된 아이템 이라면
					vecParentName.push_back(m_Tree.GetItemText(hRootItem));
					vecInfo.push_back(m_Tree.GetItemText(hItem));
				}
	...
	//GetItemState(hItem, UINT(TVIS_SELECTED)) & TVIS_SELECTED;
...
BOOL CColumnTreeCtrl::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT *pResult)
{
	...
	//m_Tree.SendMessage(WM_NOTIFY, wParam, lParam);
